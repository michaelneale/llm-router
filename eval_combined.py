...
cfg = load_config("configs/swebench-pool.yaml")
# point at combined checkpoint
cfg.routing.checkpoint = "checkpoints/combined/prefill_router.pt"
router = build_router_from_config(cfg)

# held-out: task -> {slot: isCorrect}
rows = list(csv.DictReader(open("data/combined-test-split.csv")))
